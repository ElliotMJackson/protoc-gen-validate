...
/**
 * {@code ProtoTypeMap} maintains a dictionary for looking up Java type names
 * when given proto types.
 */
public final class ProtoTypeMap {
    ...
    /**
     * Returns an instance of {@link ProtoTypeMap} based on the given
     * FileDescriptorProto instances.
     *
     * @param fileDescriptorProtos the full collection of files descriptors from the
    ...
    private static void recursivelyAddTypes(ImmutableMap.Builder<String, String> types,
            DescriptorProtos.DescriptorProto m, String protoPackage, String enclosingClassName, String javaPackage) {
    ...
    private static String getJavaOuterClassname(
        ...
        // If the outer class name is not explicitly defined, then we take the proto
        // filename, strip its extension,
        ...
        // Protos in subdirectories without java_outer_classname have their path
        // prepended to the filename. Remove
        // if present.
    ...
    /**
     * In the event of a name conflict between the outer and inner type names,
     * protoc adds an OuterClass suffix to the
     * outer type's name.
     */
    private static String appendOuterClassSuffix(final String enclosingClassName,
    ...
    /**
     * Replace invalid proto identifier characters with an underscore, so they will
     * be dropped and camel proto below.
    ...
    private static String makeInvalidCharactersUnderscores(String filename) {
        ...
        for (int i = 0; i < filechars.length; i++) {
            char c = filechars[i];
            if (!CharMatcher.inRange('0', '9').or(CharMatcher.inRange('A', 'Z')).or(CharMatcher.inRange('a', 'z'))
                    .matches(c)) {
                filechars[i] = '_';
            }
        }
